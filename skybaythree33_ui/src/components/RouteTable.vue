        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalRoutes = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalRoutes">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Route</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="RouteId" type="text" placeholder="Enter RouteId" v-model="routeToAdd.routeId"></base-input>
  <base-input label="Distance" type="text" placeholder="Enter Distance" v-model="routeToAdd.distance"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="routes" :row-key="record => record.RouteId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <RoutePictureView :routes="routes" />
          </div>
...
import RoutePictureView from './RoutePictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      sortBy: 'routeId',           // Column to sort by
  ...
  methods: {
    ...
    async renderRoutesTable() {
      ...
      for (let i = 0; i < this.routes.length; i++) {
        routesTableData.push({
          ...
          year: this.routes[i].year,
          date: this.routes[i].date,
          competitionId: this.routes[i].competitionId,
          routeId: this.routes[i].routeId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-routes',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForRoutesChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.routeToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.routeToAdd);
      console.log(jsonData);
      
      const res = await RouteService.addRoute(jsonData);
